...
class GPT2BPEVocabulary(Vocabulary):
  """The HuggingFace Vocabulary Class.
  ...
  def _decode(self, ids: Sequence[int]) -> str:
    return self._tokenizer.decode(token_ids=ids)

  def batch_encode(self, strs: Sequence[str]) -> Sequence[Sequence[int]]:
    results = self._tokenizer.batch_encode_plus(strs, padding=False)
    return results.input_ids

  def batch_decode(self, ids: Sequence[int]) -> str:
    return self._tokenizer.decode(token_ids=ids)
  ...
  def _encode_tf(self, s: tf.Tensor) -> tf.Tensor:
    ...
    s_len = len(s.shape)
    s = tf.cond(s_len, lambda: s, lambda: tf.expand_dims(s, 0))
    out = tf.py_function(
    ...
    if s_len:
      return out
    return tf.squeeze(out.to_tensor(), 0)
  ...
  def _decode_tf(self, ids: tf.Tensor) -> tf.Tensor:
    ...
    ids_len = len(ids.shape)
    ids = tf.cond(ids_len > 1, lambda: ids, lambda: tf.expand_dims(ids, 0))

    out = tf.py_function(func=_batch_decode_py_func, inp=[ids], Tout=tf.string)
    out = tf.squeeze(out, 0)
    return tf.cond(ids_len > 1, lambda: out, lambda: tf.squeeze(out, 0))
  ...
  @property
  def tf_tokenizer(self):
    raise NotImplementedError("The tf_tokenizer is not implemented.")
