...
class TasksTest(test_utils.FakeTaskTest):
  ...
  def _mock_and_assert_cached_source(self, task_name, buffer_size):
    cached_task = dataset_providers_helpers.get_mixture_or_task(task_name)
    assert isinstance(cached_task, dataset_providers.Task)
  ...
  def test_list_shards(self):
    ...
    def _get_formatted_shards_list(task_name, split):
      ...
      assert isinstance(task, dataset_providers.Task)
      shards = task.source.list_shards(split)
