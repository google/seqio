...
def get_targets_and_examples(
  ...
  for task in tasks:
    ...
    if "targets" in ds.element_spec or "targets_pretokenized" is ds.element_spec:
      for ex in tfds.as_numpy(ds):
        ...
        if "targets_pretokenized" in ex:
          ...
          target = task.output_vocabulary.decode(
  ...
  def _compute_metrics(
    ...
    for task in self.eval_tasks:
      ...
      if task.predict_metric_fns or task.predict_targetless_metric_fns:
        ...
        outputs = [
            task.output_vocabulary.decode([int(token) for token in tokens])
